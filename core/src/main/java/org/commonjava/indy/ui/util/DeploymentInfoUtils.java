...
import org.commonjava.indy.ui.boot.IndyUIDeploymentProvider;
...
public final class DeploymentInfoUtils
{
    ...
    public static void mergeFromProviders( final DeploymentInfo into, final Set<IndyUIDeploymentProvider> fromProviders,
                                           final String contextRoot, final Application application )
    {
        for ( final IndyUIDeploymentProvider fromProvider : fromProviders )
        {
        ...
        if ( filterMappings != null )
        {
            for ( final FilterMappingInfo fmi : filterMappings )
            {
                switch ( fmi.getMappingType() )
                {
                    ...
                    default:
                    {
                        logger.debug( "Found URL filter mapping: {} -> {}({})", fmi.getFilterName(), fmi.getMapping(),
                                      fmi.getDispatcher() );
